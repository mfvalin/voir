...
static void CalculerLargeurMenus(wordint largeurMenus[], wordint table[][3]);
...
wordint InitFiltresInfo(filtresInfoStruct filtresInfo[], filtresStruct *filtres, wordint nbFiltres)
{
   ...
   qsort((char *)filtres, nbFiltres, sizeof(filtresStruct), (__compar_fn_t) ComparerFiltres);
   indDesCourant = 0;
...
void InitWidgetsMenu(XmString TitresMenus[], wordint nbDes)
{
   ...
   for (j = 0; j < nbDes; j++)
      ...
      xs[wi].menus[j] = (Widget)XmCreatePushButton(xs[wi].menuform,
                                                          "menus",
                                                          args, i);
      XtAddCallback(xs[wi].menus[j],  XmNactivateCallback, (XtCallbackProc) MontrerDescripteurs, NULL);
...
void CalculerLargeurMenus(wordint largeurMenus[], wordint table[][3])
{
   wordint i,j;
   XmFontList fontListe;
   wordint largeurTexte;
...
void XSelectstdTerminerInit(wordint table[][3], wordint nbrecs)
{
   ...
   for (i = 0; i < xs[wi].nbDes; i++)
      qsort((char *)xs[wi].cles[i].cleInfo, xs[wi].cles[i].nbCles, sizeof(cleInfoStruct), (__compar_fn_t) ComparerCles);
